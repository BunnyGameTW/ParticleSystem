			...
			for (int i = 0; i < n - _iGenParticles; i++) {
				...
				if (_iFree != 0) {
					if (_iEmitterType == 0) {
						...
						get->setVelocity(3.5f);
						get->setLifetime(0.4f);						
						get->setPosition(_emitterPt);
						get->setSize(0.125f);
						setParticles(130);
					...
					/*else if (_iEmitterType == 4) {
						get = _FreeList.front();
						Point pos = Point((int)(_fElpasedTime*100)%1000+300, (int)(_fElpasedTime * 100) % 200 + 300);
						get->setBehavior(EMITTER_TWO);
						get->setPosition(pos);					
						get->setLifetime(_fLifeTime);
						get->setSize(0.125f);
		...
		if (n > _iGenParticles) {  // 產生的分子個數不足，產生到 n 個分子
			for (int i = 0; i < n - _iGenParticles; i++) {
				if (_iFree != 0) {
					if (_iSparkPart == 1) {
						get = _FreeList.front();
						Point pos=	Point((int)(_fSparkTimer * 100) % 1000 + 300, (int)(_fSparkTimer * 500) % 1000);
						get->setBehavior(EMITTER_SPARK);
						get->setPosition(pos);
						get->setLifetime(0.8f);
						get->setSize(0.5f);
						setParticles(100);
						_fGravity = 0.0f;
						_fSpread = 0.1f;
...
void CParticleSystem::setColor(float fRed, float fGreen, float fBlue) {//set red
	_color = Color3B (fRed, fGreen, fBlue);
}
...
void CParticleSystem::setSize(float fSize) {//set png
	_fSize = fSize;
	list <CParticle *>::iterator it;
	if (_iInUsed != 0) { // 有分子需要更新時
		for (it = _InUsedList.begin(); it != _InUsedList.end(); it++) {
