...
bool ParticleSystemScene::init()
{
	...
	// Slider of size
	auto *SizeSlider = (cocos2d::ui::Slider *)(rootNode->getChildByName("Slider_Size"));
	SizeSlider->addEventListener(CC_CALLBACK_2(ParticleSystemScene::SizeEvent, this));
	SizeSlider->setMaxPercent(100); 	// 將 0 到 100 對應到 0 到 255 之間
	_SizeBMValue = (cocos2d::ui::TextBMFont *)rootNode->getChildByName("SizeBMFont");
...
void ParticleSystemScene::FireworkEvent(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type) {
	if (type == Widget::TouchEventType::BEGAN) {
		_ParticleControl.setPic("comet.png", *this);
		_ParticleControl._bSpark = true;
		_ParticleControl._iSparkPart = 1; 
		_ParticleControl._fSparkTimer = 0.0f;
	}
}
void ParticleSystemScene::SizeEvent(cocos2d::Ref* sender, cocos2d::ui::Slider::EventType type) {
	if (type == Slider::EventType::ON_PERCENTAGE_CHANGED)
	{
		Slider* slider = dynamic_cast<Slider*>(sender);
		int percent = slider->getPercent();
		int maxPercent = slider->getMaxPercent();
		int iType = percent / 50; // 0 到 5 之間
		_SizeBMValue->setString(StringUtils::format("%2d", iType));
		_ParticleControl.setSize(iType);
	}
}
